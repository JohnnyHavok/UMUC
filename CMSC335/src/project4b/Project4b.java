...
public class Project4b extends JFrame implements ActionListener {
	// Constants
	...
	// Data Structures
	Library<SpecificGenreBookInterface>	library 		= new Library<SpecificGenreBookInterface>();
	...
	public enum BookType {
		...
		public static SpecificGenreBookInterface getBook(BookType bt) {
			switch(bt) {
	 ...
	 * and based on the source of an incomcing event we do certain actions.
	 */
	...
	public void actionPerformed(ActionEvent source) {
		...
		if(source.getSource() == (JButton) btAdd) {
			...
			if(make) { // make goes false if something is missing from the text fields.
				try {
					SpecificGenreBookInterface b = BookType.getBook((BookType) listGenres.getSelectedItem());
					b.setAuthor(author);
					b.setTitle(title);
					b.setISBN(isbn);
					b.setAttribute(attr);
	 ...
	 * Input is an integer that cooresponds to the index of the row to remove.
	 * @param i
	 */
	 ...
	 * @param b
	 */
	public void addRow(SpecificGenreBookInterface b) {
	...
	public void loadLibrary(File filehandle) {
		...
		try {
			...
			if(read instanceof Library<?>) {
				...
				library = (Library<SpecificGenreBookInterface>) read;
				// Add books to bookTable's rowElement Vector
				Object[] bArray = library.toArray();
				for(Object b : bArray)
					addRow((SpecificGenreBookInterface) b);
			} else {
